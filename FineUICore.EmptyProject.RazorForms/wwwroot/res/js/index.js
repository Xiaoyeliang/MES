...
function activeTabAndRefresh(tabId) {
    ...
    if (targetTab) {
        mainTabStrip.activateTab(targetTab);
    ...
    if (nodePaths && nodePaths.length) {
        ...
        $.each(nodePaths, function (index, item) {
            if (item === 'root') {
                ...
                //var itemData = treeInstance.getNodeData(item);
                //var itemHtml = itemData.text;
                var itemHtml = treeInstance.getNodeEl(item).find('.f-tree-cell-text').html();
             
              // console.log('<span class="' + cls + '">' + itemHtml + '</span>');
...
F.ready(function () {
    ...
    // options.moveToEnd: 将新增选项卡移到尾部（如果选项卡已存在，则不改变位置）（默认值：true）
    ...
    var initOptions = {
        ...
        beforeTabAdd: function (event, tabOptions, treeNodeId) {
            // 注意：此时尚未创建选项卡，所以第二个参数为tabOptions而不是tabInstance
